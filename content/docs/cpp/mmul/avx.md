...
これは，直列に実行する処理を並列に行うことで，実行時間の短縮を図ったものですが，同様の効果を狙う別の方法として，SIMD (Single Instruction, Multiple Data) と呼ばれる方法があります．

この方法は，CPU拡張命令を用いて実現できます．
Intel CPU では，Intel Intrinsics API という，CPUの命令セット拡張へのアクセスができるAPIを公開しています．

このAPIは以下のページで公開されています [[1]](#intel:1)．

C++17 では，例えば `<immintrin.h>` をインクルードすることで，AVX2 という拡張機能が使えて，コンパイル時に `-march=native -mavx2` というフラグを追加すると，AVX2 による命令が実行できます．

本ページでは，Intel CPU の拡張命令を用いて，行列積演算の実行時間が短縮できることを確認します．

## CPU拡張命令の利用

まず，実験用PCで，AVX2による命令が実行できるかを確かめます．
...
倍精度浮動小数点数は 64 ビットなので，1つの拡張命令で，{{< katex >}}256/64=4{{< /katex >}} 個をまとめて演算できます．

ところで，このようなCPU拡張命令を利用する場合，演算対象の変数が格納されているアドレスが，"キリのよい値" である必要があります．
AVX2 では，256ビット，つまり32バイトの倍数でなければなりません．
このような，変数のアドレスに関する制約や，その制約に合わせて調整をすることを **アライメント** と呼びます．
...
とすることで，先頭アドレスが32バイトの倍数になるように配置されます．

`std::array<double, const_val::kSize * const_val::kSize>` も同様です．

```cpp
alignas(32) std::array<doubl, const_val::kSize * const_val::kSize> a;
```

一方で，実行時に動的にメモリを確保する `new` で確保する方法や，`std::vector` を使う方法の場合，動的に確保した配列要素のアライメント指定は可能のようですが，標準的な方法がわからなかったため，本ページでは省略します．

さて，AVX2を使って，どのように行列積演算が効率化できるかを考えます．
...
{{< katex >}}c^{[i,j]}\,(i=0,4,8,\dots,n-4,\,j=0,1,\dots,n-1){{< /katex >}} を計算すると {{< katex >}}C{{< /katex >}} のすべての要素が計算できることになりますので，以下の3つの演算が1つの命令で実現できれば，{{< katex >}}c^{[i,j]}{{< /katex >}} は効率的に計算できそうです．
...
これらは，`_mm256_broadcast_sd`, `_mm256_mul_pd`, `_mm256_add_pd` で実現できます．
ここに，AVX2で演算するために要素を読み込む `_mm256_load_pd`, 逆に演算結果を指定のアドレスに書き出す `_mm256_store_pd` をあわせると，AVX2 による行列積演算の実装ができるようになります．

例えば，AVX2を使わない実装が以下のとおりとします (`sdim_raw_array`)．
...
これを，AVX2 を利用する場合は，以下のようになります (`sdim_raw_array_avx`)．
...
ところで，ループ展開という手法でさらに実行時間の短縮を図ることができます．
...
前節の議論から，生の1次元配列と `std::array` による1次元配列で，以下の合計6種類の方法について，前ページと同様の方針で，実行時間を比較します．
...
最適化レベル1以上をみると，AVX2を使うと3ミリ秒（0.003秒），ループ展開まで使うと2ミリ秒（0.002秒）まで実行時間が短縮できていることがわかります． 

特に，ループ展開によって実行時間が短くなっていることもわかります．
...
したがって，Intel CPU の拡張命令による実行時間の短縮効果が観測できたといえます．
