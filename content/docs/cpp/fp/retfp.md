...
結論からいうと，文字列を受け取って，`(double, double) -> double` の関数へのポインタを返す関数は，以下のような（奇妙な）宣言になることがわかります．
...
インタフェースクラス `Operator` が `Compute` メソッドの定義を要求し，移譲された継承クラスでそれぞれ具体的なロジックを定義しています．

`From` 関数では，ヒープ領域上にスマートポインタで，指定された文字列に対応するオブジェクトを生成して返しています．
...
関数ポインタの型は，どのように変数を宣言するかを考えるとわかりやすいので，その方法で復習します．
`double f(double, double)` という関数へのポインタなので，変数は `double (*f)(double, double)` と
宣言するのでした．

つまり，以下のソースコードはコンパイルできて，意図通りに実行できます．
...
関数ポインタの変数は `double (*)(double, double) f;` と**書けず**，`double (*f)(double, double);` となるのでした．

関数ポインタを返す関数の場合も `double (*)(double, double) From(const std::string& label);` とは**書けません．**
