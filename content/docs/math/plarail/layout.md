...
タカラトミーのプラレールという鉄道玩具が広く知られています [[1]](#takaratomy:1)．[前ページ]({{< ref rail.md >}}) では，プラレールのレールに関する形式的定義を与えました．本ページではレイアウト（線路配線）の数学的表現を考えます．
...
レールを頂点としたグラフを考えることで，レールごとの接続の仕方が表現できます．接続の仕方が決まると形状が決まりますが，これは各レールの端点や分岐点を頂点とする単純グラフで表現できます．以下，接続を表すグラフを **接続グラフ**，形状を表すグラフを **形状グラフ** とよぶことにします．
これらをふまえて，レイアウトの形式的定義を考えます．
...
2. \\(\tilde{\mathcal{A}}\\) を，以下を満たす \\(R\\) 全体の集合とし，\\(\tilde{\mathcal{A}}\\) の元 \\(R\\) を **擬レイアウト** という．
	...
	\\[ \lim\_{x\to l_1}\frac{{\phi\'}\_{k_1}^{i_1^k}}{\\|{\phi\'}\_{k_1}^{i_1^k}\\|\_2}=\lim_{x\to l_2}\frac{{\phi\'}_{k_2}^{i_2^k}}{\\|{\phi\'}\_{k_2}^{i_2^k}\\|\_2} \\]
	が成り立つ．
...
接続情報 {{< katex >}}c\in\Gamma_m{{< /katex >}} はどのレール同士をどの端点で接続するかの情報を表します．{{< katex >}}c=(c_1,\dots,c_m){{< /katex >}}, {{< katex >}}c_1=((i_1,i_2),((j_{11},j_{12}),\dots,(j_{l1},j_{l2}))){{< /katex >}}なら，{{< katex >}}c_1{{< /katex >}} は，
...
といった要領で接続することを表します．そして，{{< katex >}}c_i\,(i=2,\dots,m){{< /katex >}} についても，同様となります．
...
{{% hint info %}}
{{< theorem-label name="Definition" >}} \\(R\\) を擬レイアウトとする．\\(R\\) が接続可能かつ重なりがないとき，\\(R\\) を **レイアウト** と呼ぶ．レイアウト全体の集合を \\(\mathcal{A}\\) で表す．
{{% /hint %}}
...
完全レイアウトとは，配線結果が環状になっていて，無理なく接続されている状況を表します．一方で，{{< katex >}}(\delta,\delta'){{< /katex >}}-レイアウトは，多少離れている端点を強引につなぐことで環状にできるもので，いわゆる **なじませつなぎ** によるレイアウトを表現しています．{{< katex >}}\delta{{< /katex >}} は端点間の距離，{{< katex >}}\delta'{{< /katex >}} は端点における向きのずれ（向きのなす角の正接 (tan) の絶対値）を表します．{{< katex >}}\delta\approx0{{< /katex >}} とは端点の距離が小さく，比較的無理なくつなげていることをあらわし，{{< katex >}}\delta'\approx0{{< /katex >}} とは端点における向きのずれも小さいことを表します．逆に端点どうしの距離が小さくても，直交する場合は接続できませんが，この場合は {{< katex >}}\delta'=\infty{{< /katex >}} になります．
